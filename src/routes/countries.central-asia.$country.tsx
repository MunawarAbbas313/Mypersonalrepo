...
import { CENTRAL_ASIA_COUNTRIES } from "@/data/regions/central-asia_countries";
...
export const Route = createFileRoute("/countries/central-asia/$country")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...
      { name: "description", content: loaderData?.description ? `${loaderData.description} AL ARBAB TRAVEL AND TOURS Islamabad — Pakistan's #1 visa agency. 98% approval rate, IATA-accredited, Blue Area office.` : "" },
      { name: "keywords", content: `${loaderData?.keywords || ""}, best central asia visa consultant islamabad, top central asia visa agent pakistan, central asia visa success rate pakistan, central asia visa from pakistan 2026, AL ARBAB TRAVEL AND TOURS central asia visa, blue area visa consultant` },
      { name: "robots", content: "index, follow, max-snippet:-1, max-image-preview:large" },
      { name: "author", content: "AL ARBAB TRAVEL AND TOURS" },
      { name: "geo.region", content: "PK-IS" },
      { name: "geo.placename", content: "Islamabad" },
      { property: "og:title", content: `${loaderData?.name || "Central Asia"} Visa Consultant Islamabad — Pakistan's #1 | AL ARBAB TRAVEL AND TOURS` },
      { property: "og:description", content: loaderData?.description || "" },
      { property: "og:url", content: `https://alarbabtravels.pk/countries/central-asia/${loaderData?.slug || ""}` },
      { property: "og:type", content: "website" },
      { property: "og:locale", content: "en_PK" },
      { property: "og:site_name", content: "AL ARBAB TRAVEL AND TOURS — Pakistan's No.1 Travel Agency" },
      { property: "og:image", content: loaderData?.image || "" },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "twitter:title", content: `${loaderData?.name || ""} Visa Consultant Islamabad | AL ARBAB TRAVEL AND TOURS` },
      { name: "twitter:description", content: loaderData?.description || "" },
      { name: "twitter:image", content: loaderData?.image || "" },
    ],
    links: [
      { rel: "canonical", href: `https://alarbabtravels.pk/countries/central-asia/${loaderData?.slug || ""}` },
    ],
  }),
...
function CentralAsiaCountryPage() {
  ...
  const jsonLd = {
    ...
    name: `${country.name} Visa Consultancy — AL ARBAB TRAVEL AND TOURS Islamabad`,
    description: country.description,
    url: `https://alarbabtravels.pk/countries/central-asia/${country.slug}`,
    provider: {
      "@type": "TravelAgency",
