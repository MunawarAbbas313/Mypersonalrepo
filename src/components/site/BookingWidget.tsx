...
export function BookingWidget({ initialTab = "visa" }: { initialTab?: string }) {
  ...
  const tabs = [
    { id: "visa", label: "Visa", icon: Stamp, color: "text-[#0D47A1]" },
    { id: "flights", label: "Flights", icon: Plane, color: "text-blue-500" },
    { id: "umrah", label: "Umrah", icon: Moon, color: "text-[#F7941D]" },
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? "text-[#0D47A1]" : "text-muted-foreground hover:text-foreground hover:bg-white/50"
                    )}
                  >
                    ...
                    {isActive && (
                      <motion.div
                        ...
                        className="absolute bottom-0 left-3 right-3 h-[3px] bg-[#F7941D] rounded-t-full shadow-[0_-3px_8px_rgba(247,148,29,0.4)]"
                      />
                    )}
                  </button>
                );
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex items-center justify-end mt-6 sm:mt-8 pt-4 border-t border-border/40">
                  <Button
                    ...
                    className="w-full md:w-auto h-12 md:h-14 px-8 md:px-12 rounded-[1.25rem] bg-[#F7941D] hover:bg-[#0D47A1] text-white shadow-lg hover:shadow-xl hover:shadow-orange-500/30 hover:-translate-y-0.5 transition-all duration-300 font-bold text-sm md:text-base border-none disabled:opacity-50"
                  >
            ...
            {activeTab === "visa" && (
              ...
              >
                <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4">
                  <div className="relative group flex flex-col sm:col-span-2">
                    <Label className="text-[11px] font-black uppercase tracking-[0.15em] text-[#0D47A1] mb-2 ml-3">Destination Country</Label>
                    <div className="relative">
                      <MapPin className="absolute left-5 top-1/2 -translate-y-1/2 text-[#F7941D] z-10" size={20} />
                      <Input
                        autoComplete="off"
                        ...
                        className="h-14 md:h-16 pl-14 bg-white border-2 border-border/60 hover:border-[#0D47A1]/40 focus-visible:border-[#0D47A1] rounded-[1.25rem] text-base font-bold text-[#0D47A1] shadow-sm transition-all focus-visible:ring-4 focus-visible:ring-[#0D47A1]/10 placeholder:text-muted-foreground/40"
                      />
                      {visaSearch && (
                        <button onClick={() => { setVisaSearch(""); setShowVisaSuggestions(false); }} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground/40 hover:text-destructive z-10 p-1 bg-white">
                          <X size={18} />
                        </button>
                      )}
                      {showVisaSuggestions && filteredVisaCountries.length > 0 && (
                        <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white/95 backdrop-blur-3xl rounded-[1.25rem] shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-border/50 z-[110] overflow-hidden py-3 animate-in fade-in max-h-[300px] overflow-y-auto custom-scrollbar">
                          {filteredVisaCountries.map((country) => (
                            <button
                              ...
                              className="w-full px-5 py-3 text-left hover:bg-slate-50 flex items-center gap-4 group/item transition-all border-l-2 border-transparent hover:border-[#0D47A1]"
                            >
                              <img 
                                ...
                                loading="lazy"
                                decoding="async"
                                className="w-7 h-auto object-cover rounded-[3px] shadow-sm shrink-0 group-hover/item:scale-110 transition-transform" 
                              />
                              <span className="text-[15px] font-bold text-foreground group-hover/item:text-[#0D47A1] transition-colors truncate">{country.name}</span>
                            </button>
                          ))}
                        </div>
                      )}
                  ...
                  <div className="relative group flex flex-col">
                    <Label className="text-[11px] font-black uppercase tracking-[0.15em] text-[#0D47A1] mb-2 ml-3">Visa Type</Label>
                    <Select value={visaType || "tourist"} onValueChange={setVisaType}>
                      <SelectTrigger className="h-14 md:h-16 px-5 w-full bg-white border-2 border-border/60 hover:border-[#0D47A1]/40 focus:border-[#0D47A1] rounded-[1.25rem] text-base font-bold text-[#0D47A1] shadow-sm transition-all focus:ring-4 focus:ring-[#0D47A1]/10">
                        <SelectValue placeholder="Select Type" />
                      </SelectTrigger>
                      <SelectContent className="rounded-2xl border-white/50 bg-white/95 backdrop-blur-xl shadow-2xl p-2">
                        <SelectItem value="tourist" className="rounded-xl font-bold py-3 cursor-pointer hover:bg-slate-50 transition-colors">Tourist Visa</SelectItem>
                        <SelectItem value="business" className="rounded-xl font-bold py-3 cursor-pointer hover:bg-slate-50 transition-colors">Business Visa</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>

                  <div className="relative group flex flex-col">
                    <Label className="text-[11px] font-black uppercase tracking-[0.15em] text-[#0D47A1] mb-2 ml-3">Contact No.</Label>
                    <div className="relative">
                      <Phone className="absolute left-5 top-1/2 -translate-y-1/2 text-[#F7941D] z-10" size={20} />
                      <Input placeholder="WhatsApp No." type="tel" value={userPhone} onChange={(e) => setUserPhone(e.target.value)} className="h-14 md:h-16 pl-14 bg-white border-2 border-border/60 hover:border-[#0D47A1]/40 focus-visible:border-[#0D47A1] rounded-[1.25rem] text-base font-bold text-[#0D47A1] shadow-sm transition-all focus-visible:ring-4 focus-visible:ring-[#0D47A1]/10 placeholder:text-muted-foreground/40" />
                    </div>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row items-center justify-end gap-4 mt-8 sm:mt-10 pt-6 border-t-2 border-border/30">
                  <Button
                    onClick={() => window.open(`https://wa.me/92315323811?text=${encodeURIComponent(`Hello! I would like to inquire about visa services for ${visaSearch}.`)}`, "_blank")}
                    disabled={!isVisaValid()}
                    className={cn(
                      "w-full sm:w-auto h-14 md:h-16 px-8 rounded-[1.25rem] font-black text-[13px] md:text-[15px] uppercase tracking-wide transition-all flex items-center gap-3 border-none",
                      isVisaValid() ? "bg-[#25D366] hover:bg-[#1EBE5D] text-white shadow-[0_10px_30px_rgba(37,211,102,0.25)] hover:shadow-[0_15px_40px_rgba(37,211,102,0.4)] hover:-translate-y-1" : "bg-muted text-muted-foreground/40 cursor-not-allowed"
                    )}
                  >
                  ...
                  <Button
                    ...
                    disabled={!isVisaValid()}
                    className="w-full sm:w-auto h-14 md:h-16 px-10 rounded-[1.25rem] bg-gradient-to-r from-[#0D47A1] to-[#29ABE2] hover:opacity-90 text-white shadow-[0_10px_30px_rgba(13,71,161,0.25)] hover:shadow-[0_15px_40px_rgba(13,71,161,0.4)] hover:-translate-y-1 transition-all font-black text-[13px] md:text-[15px] uppercase tracking-wide border-none disabled:opacity-50"
                  >
                    <Stamp className="mr-2" size={22} />
                    Start Application
                  </Button>
                </div>
              </motion.div>
            ...
            {activeTab === "hotel" && (
              ...
              >
                ...
                <div className="flex items-center justify-end mt-6 sm:mt-8 pt-4 border-t border-border/40">
                  <Button
                    onClick={() => window.open(getInquiryMessage("+92315323811"), "_blank")}
            ...
            >
              ...
              <div className="p-5 sm:p-7 space-y-4 overflow-y-auto custom-scrollbar">
                <div className="bg-secondary/20 rounded-3xl p-4 border border-border/50 shadow-inner">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {(activeTab === 'umrah' ? [
                      { name: "AL ARBAB TRAVEL AND TOURS Expert", role: "Umrah Specialist", phone: "+92315323811" }
                    ] : activeTab === 'flights' ? [
                      { name: "AL ARBAB TRAVEL AND TOURS Expert", role: "Ticketing Officer", phone: "+92315323811" }
                    ] : [
                      { name: "AL ARBAB TRAVEL AND TOURS Expert", role: "Visa Specialist", phone: "+92315323811" },
                      { name: "AL ARBAB TRAVEL AND TOURS Expert", role: "Service Representative", phone: "+92315323811" }
                    ]).map((rep) => (
                      <a
